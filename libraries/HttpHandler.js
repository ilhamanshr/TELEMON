...
exports.httpRequest = function(reqId, clientIp, isSSL, options, params, headers, cb) {
    ...
    if (options.API_HOST != "sso.opqrs.xyz") {
        ...
        res.on('end', function() {
            if (result) {
                try {
                    ...
                    if (options.API_HOST != "sso.opqrs.xyz") {
                        logger.debug(__filename, JSON.stringify(result), reqId, clientIp, "Response received from "+ ((isSSL) ? "https://" : "http://") + httpOptions.host +":"+ (("port" in httpOptions) ? httpOptions.port : "") + (("path" in httpOptions) ? httpOptions.path : ""));
                    }
                    ...
                    if (result.hasOwnProperty("code")) {
                        ...
                        response["message"] = response.message +". Code not exist.";
                        cb(response);
                    }
                } catch (err) {
                    if (options.API_HOST != "sso.opqrs.xyz") {
                    ...
                    var response = utils.duplicateObject(msg.ERR_BAD_GATEWAY);
                    response["message"] = response.message +". Response from backend: "+ result;
                    cb(response);
                }
            } else {
                if (options.API_HOST != "sso.opqrs.xyz") {
