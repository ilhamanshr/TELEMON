...
router.post('/api', function(req, res) {
    ...
    if (bodyReq.hasOwnProperty("token") && bodyReq.hasOwnProperty("permission") && bodyReq.token && bodyReq.permission) {
        var file = './controllers/'+ bodyReq.action +'.js';
        
        try {
            if (fs.existsSync(file)) {
                var ctrl = require(BASE_DIR + '/controllers/' + bodyReq.action);
